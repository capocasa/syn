...
type
  StateVariableFilter* = object
    g*, k*, a1*, a2*, a3*: float32    ## filter coefficients
...
proc setCoefficients*(filter: var StateVariableFilter, sampleRate, freq, Q: float64) =
  ...
  filter.g = fastTan((PI * freq / sampleRate).float32)
  filter.k = (1.0 / Q).float32
  filter.a1 = 1.0f32 / (1.0f32 + filter.g * (filter.g + filter.k))
...
proc bell*(filter: var StateVariableFilter, sampleRate, freq, Q, dbGain: float64) =
  ...
  let A = fastPow10(dbGain)
  filter.g = fastTan((PI * freq / sampleRate).float32)
  filter.k = (1.0 / (Q * A.float64)).float32
  filter.a1 = 1.0f32 / (1.0f32 + filter.g * (filter.g + filter.k))
  filter.a2 = filter.g * filter.a1
  filter.a3 = filter.g * filter.a2
  filter.m0 = 1.0f32
  filter.m1 = filter.k * (A*A - 1.0f32)
  filter.m2 = 0.0f32

proc lowShelf*(filter: var StateVariableFilter, sampleRate, freq, Q, dbGain: float64) =
  ## Configure as low shelf filter
  let A = fastPow10(dbGain)
  filter.g = fastTan((PI * freq / sampleRate).float32) / sqrt(A)
  filter.k = (1.0 / Q).float32
  filter.a1 = 1.0f32 / (1.0f32 + filter.g * (filter.g + filter.k))
  filter.a2 = filter.g * filter.a1
  filter.a3 = filter.g * filter.a2
  filter.m0 = 1.0f32
  filter.m1 = filter.k * (A - 1.0f32)
  filter.m2 = (A*A - 1.0f32)
...
proc highShelf*(filter: var StateVariableFilter, sampleRate, freq, Q, dbGain: float64) =
  ...
  filter.a3 = filter.g * filter.a2
  filter.m0 = A * A
  filter.m1 = filter.k * (1.0f32 - A) * A
  filter.m2 = (1.0f32 - A*A)
...
proc next*(filter: var StateVariableFilter, v0: float32): float32 =
  ...
  filter.ic1eq = 2.0f32 * v1 - filter.ic1eq
  filter.ic2eq = 2.0f32 * v2 - filter.ic2eq
